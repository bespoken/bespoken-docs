...
![Prerecorded audio](../assets/images/faq/faq-prrecorded-audio.png)
...
### The transcript response from my test differs from what I'm expecting. What to do?
For functional tests that use speech recognition, it is common to get some words, acronyms and punctuation wrong. To compensate for this, there are two settings can be adjusted to make tests pass.

#### Assertion Fuzzy Threshold
This property is common to all testing platforms. It represents how forgiving we should be when comparing the expected vs actual values received expressed as decimal number from 0 to 1. Setting this property to 1 means the values have to match exactly, lowering the value makes the assertions more and more forgiving.

Here's an example setting this value to 1. Notice how the assertion failed when the only difference was "you are" vs "you're" at the end and the test still fails.

![Fuzzy Treshold set to 1](../assets/images/faq/faq-fuzzy-1.png)

Here's an example setting this value to 0.8 (our default value). Notice how this time the test passes even while expecting "you are". There's also another difference "Contact Center" vs "Customer Center".

![Fuzzy Treshold set to 0.8](../assets/images/faq/faq-fuzzy-08.png)

In both cases, punctuation signs are not considered when evaluatig results.

#### Homophones
Homophones are two or more words that share the same pronunciation, but which have different spellings or meanings. For example, the words ‘hear’ and ‘here’ are homophones because they mean completely different things, even though they sound similar. Homophones can be hard to differentiate during speech recognition, to fix this you can specify a list of common homophones under the advanced settings like this:

![Homophones](../assets/images/faq/faq-homophones.png)

In this case, any of the comma delimited values will be replaced by it's key before evaluating the results. 

### How can I execute a subset of my tests within the Dashboard?
If you have a big test suite that contains multiple tests and only want to run a subset of them, simply mark the tests you want to run using the "only" flag. To do this, click on the three dots next to the names of the tests you want to select and click on "Only" option. After this, click on "Run All" and Bespoken will only run the tests marked with this option, ignoring the rest.

![Only](../assets/images/faq/faq-only.gif)

Alternatively, you can use the "Skip" option to mark the tests that should not be run when clickin on "Run All".
...
Of course, make sure the Alexa account you are using is the same one that your virtual device is associated with.
...
By default, our virtual devices will always point to the US region. If you want to reach a voice app in a specific region you'll need to create a virtual device tied to an Amazon account that is specific to the region that you want to test. That is, an amazon.co.uk for testing a voice app published in the UK, an amazon.es account for voice apps in Spain, and so on. 
