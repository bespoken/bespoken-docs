...
Bespoken can interact with your web chatbots to test their functionality interacting with it like a real user would and evaluating its responses.
...
2. He then clicks on the "Ask Pepper" button at the bottom.
...
Now, here's how this would look like as a Bespoken test:

![Webchat test](../assets/images/guides/webchat-test.gif)

Notice how:  
- We entered the URL as the main configuration parameter
- The inputs in the first two interactions are plain text which then will be typed in the chatbot
...
| --- | --- | --- |  
| URL | URL in which the chatbot is located. | N/A |  
...
This code looks for a button that contains the text "Rewards program" and clicks on it. While this is not the norm, you can use this technique to press buttons, select values from a dropdown, check radiobuttons and more.
...
| Reply Settings: Script After Each Command | Miliseconds to wait for a reply. | 15000 |
| Reply Settings: IFrame Selector | IFrame CSS selector, if the chatbot lives within one. | N/A |
| Load Timeout | Miliseconds to wait for the webpage to finish loading. | 10000 |
| Initialize Timeout | Miliseconds to wait for the initialize script to finish. | 5000 |
...
For reference, these are the values used on the Chipotle example above:
...
## Video evidence 
After each test run, you'll have video playback available at the left side of the testing page. This can be helpful as  evidence of your test and can also be used during the test setup to better understand what your test is doing and correct any step that's not working as expected.
