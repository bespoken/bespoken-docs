...
| deviceLocation | An object to set a specific location value from where the requests are sent in Google Virtual Devices. It is composed by the `lat` and `lng` keys. Read [here](./faq#how-do-i-change-my-virtual-device-location-to-test-location-specific-features) to know how to use it.
