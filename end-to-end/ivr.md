...
|runInBand|If set to `true`, it will run all test suites (YAML files) serially, it means one test suite will start after the previous one has been completed - can be either `true` or `false`|boolean|Global|true|
